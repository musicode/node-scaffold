...
describe('test/service/trace/follow.test.js', () => {
  ...
  describe('follow service', () => {
    ...
    it('follow question', async () => {
      ...
      // 作者收到提醒的数量
      let followRemindCount = await trace.follow.getFollowQuestionRemindCount(user1.id)
      assert(followRemindCount === 0)

      // 作者收到未读提醒的数量
      let followUnreadRemindCount = await trace.follow.getFollowQuestionUnreadRemindCount(user1.id)
      assert(followUnreadRemindCount === 0)
      ...
      await trace.follow.followQuestion(question.id)
      ...
      followRemindCount = await trace.follow.getFollowQuestionRemindCount(user1.id)
      assert(followRemindCount === 1)
      ...
      // 标记已读
      await trace.follow.readFollowQuestionRemind(user1.id)
